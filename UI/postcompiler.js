...
PostCompiler.ShowCode = function(code){
	...
	try {		
		for (var i = 0; i < cnc.ProgramCode.length; i++) {
		    ...
		    if (line.command == CNC.GCommands.G){
				lxx = line.x;
				lyy = line.y;
				lzz = line.z;
			}
...
PostCompiler.NormalizeCode = function(code){
    if (cnc.currentStats.minX != 0 || cnc.currentStats.minY != 0){
        cnc.ProgramCode.each(function(line){
            if (line.command == CNC.GCommands.G){
                line.x -= cnc.currentStats.minX;
                line.y -= cnc.currentStats.minY;
            }
        });
    }
    PostCompiler.ShowCode(cnc.ProgramCode);
