...
Inherit(CncController, EventEmitter,{
	Init : function (settings) {
	    this.Device = {};
	    ServiceProxy.Connect("ws://localhost:5700/CNCService", {}).then(function(service){
		        console.log('service connected');
		        this.Device = service;
		        this.Device.on('cnc connected', function(){Channels.emit("device.connected", message);});
		        
		        this.Device.connectCNC('COM7', 115200).then(function(){console.log('cnc connected')}).catch(function(error){console.log(error);});
...
*/
	...
	ProgCommand : function (str, callback) {
		if (typeof (str) == "strinssendCommand") {
			...
			this.Device.sendCommand({command: str});/*
		if (str){
