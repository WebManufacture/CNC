...
CncUI.QuickProgram = function(fpath){
	this.LoadProgram(fpath, function(text){
		...
		var code = PostCompiler.ProcessCode(text);
		if (code){
			cnc.SendProgram(code);
		}	
	});	
};

CncUI.LoadCurrent = function(){
	ProgStorage.GET("current.cnc", function(code){
		cnc.ProgramCode = CncCompiler.Compile(cnc.Settings, code, { x: lx, y : ly, z: lz, speed : cnc.Settings.speed });;
	    PostCompiler.ShowCode(cnc.ProgramCode);
		Preview.ShowCode(cnc.ProgramCode);
	});
};

CncUI.SaveCurrent = function(){
	var win = this;
	if (cnc.ProgramCode){
    	//var text = JSON.stringify(cnc.ProgramCode);
    	var text = "";
    	cnc.ProgramCode.each(function(line){
    	    if (line.command == CNC.GCommands.G || line.command == CNC.GCommands.M){
    	        text += CNC.CommandsShort[line.command] + " " + line.x + " " + line.y + " " + line.z + " " + line.speed + "\n";
    	    }
    	    else{
    	        text += CNC.CommandsShort[line.command] + " " + line.speed + "\n";
    	    }
    	});
    	ProgStorage.POST("current.cnc", text, function(){
    		console.log("saved");
    	});
	}
	else{
