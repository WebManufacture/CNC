...
Inherit(CNCService, Service, {
    ...
    move: function (command) {
        ...
        var interpolation = '';
        if (command.units){interpolation = 'G91'}else {interpolation='G90'};
        var prog = [];
        prog[0] = interpolation+'\n';
        var c = 'G0';
        var nextPosition;
        ...
        var command = {type:'program', program: prog};
            ...
            switch (typeof (command)) {
                ...
                case 'object':
                    ...
                    if (!commands[command.type]) {
                        if (command.type == 'spindle1'){self.sendCode('M114\n')};
    ...
    stateParser: function(message){
        ...
        console.log(message)
        var regex = /<([A-Za-z]+),X:([-]?\d+.\d+),([-]?\d+.\d+),([-]?\d+.\d+)/;
